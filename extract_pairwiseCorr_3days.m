function [PWCorr1_2, PWCorr2_3, PWCorr1_3] = extract_pairwiseCorr_3days(mouse_session_folder, sig_threshold, only_place_cells)
...
for cell_i = 1:size(cell_to_index_map)
    ...
    if cell_to_index_map(cell_i,1) > 0 && cell_to_index_map(cell_i,3) > 0
        if place_cells{1}(cell_to_index_map(cell_i,1)) == 1 || place_cells{3}(cell_to_index_map(cell_i,3)) == 1
            cellIndex1_3(end+1,:) = cell_i;
        end
    end
end
...
if ~isempty(cellIndex1_3)
    ...
    % Compute current day population vector
    PV1 = [];
    for cell_i = 1:length(cellIndex1_3)
        cell2pick = cell_to_index_map(cellIndex1_3(cell_i),day);
        current_map = tuning_map(cell2pick).likelihood;
        current_map = smoothPlaceField(current_map);
        PV1(:,cell_i) = current_map(:);
    end
    ...
    %% PVcorr for day 2-3\
    for cell_i = 1:length(cellIndex1_3)
        [PWCorr1_3(cell_i), p_value] = corr(PV1(:,cell_i),PV3(:,cell_i));
        if ~isempty(sig_threshold)
            if p_value > sig_threshold
                PWCorr1_3(cell_i) = NaN;
            end
        end
    end
else
