...
[~, MI, PDF, occupancy_vector, prob_being_active, tuning_curve ] = extract_1D_information(binarized_trace, acceleration, acceleration_bin_vector, inclusion_vector);
...
for k = 1:numShuffles
    ...
    [~, ~, ~, ~, ~, shuffled_tuning_curve(:,k)] = extract_1D_information(shuffled_binarized, acceleration, acceleration_bin_vector, running_ts);
end 
...
for k = 1:numShuffles
    ...
    [~, ~, ~, ~, ~, actual_bootstrap_tuning_curve(:,k) ] = extract_1D_information(binarized_trace, acceleration, acceleration_bin_vector, bootstrap_ts);
    
    %% Compute the shuffled tuning curve using the same bootstrapped sample
    [~, ~, ~, ~, ~, shuffled_bootstrap_tuning_curve(:,k)] = extract_1D_information(shuffled_binarized, acceleration, acceleration_bin_vector, bootstrap_ts);
end
