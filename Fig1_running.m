...
inclusion_vector = running_ts; % Only include right runs
...
min_acceleration  = min(included_acceleration)+.4;
max_velocity = max(included_velocity)-2; % Set these to the borders of the maze used. !!! Has to be the same for every mouse for consistent MI computation !!!
max_acceleration = max(included_acceleration)-.3;

velocity_binsize = (max_velocity-min_velocity)/256;
acceleration_binsize = (-(min_acceleration-max_acceleration))/256;

%velocity_bin_vector =
%acceleration_bin_vector =  
...
for k = 1:numShuffles
    ...
    [~, ~, ~, ~, ~, actual_bootstrap_tuning_curve(:,k) ] = extract_1D_information(binarized_trace, interp_behav_vec, bin_vector, bootstrap_ts);
    
    %% Compute the shuffled tuning curve using the same bootstrapped sample
    [~, ~, ~, ~, ~, shuffled_bootstrap_tuning_curve(:,k)] = extract_1D_information(shuffled_binarized, interp_behav_vec, bin_vector, bootstrap_ts);
end
